...
const ProtectedRoute = ({ children, requireOnboarding = false }) => {
  const { user, isAuthenticated, loading: authLoading } = useAuth();
  const { isCompleted, loading: onboardingLoading } = useOnboarding(); // Retiré loadOnboardingData
  const location = useLocation();
  const [initialCheckDone, setInitialCheckDone] = useState(false);

  // Marquer la vérification initiale comme terminée une fois que l'utilisateur est authentifié
  useEffect(() => {
    if (isAuthenticated && user && !initialCheckDone) {
      setInitialCheckDone(true);
    }
